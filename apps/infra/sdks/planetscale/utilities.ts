...
export async function getPackage() : Promise<string | undefined> {
	if (_packageRef === undefined) {
		...
		if (_packageRef === undefined) {
			...
			params.setName("planetscale");
			params.setVersion("0.6.1");
			params.setValue(Uint8Array.from(atob("eyJyZW1vdGUiOnsidXJsIjoicmVnaXN0cnkub3BlbnRvZnUub3JnL3BsYW5ldHNjYWxlL3BsYW5ldHNjYWxlIiwidmVyc2lvbiI6IjAuNi4xIn19"), c => c.charCodeAt(0)));

			const req = new resproto.RegisterPackageRequest();
			req.setName("terraform-provider");
			req.setVersion("0.12.0");
			req.setDownloadUrl("");
